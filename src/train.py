...
def build_dataloaders(config: Dict) -> Tuple[DataLoader, DataLoader]:
    ...
    train_samples, val_samples = split_train_val(
        samples,
        val_size=config["experiment"].get("val_size", 0.2),
        seed=config.get("seed", 42),
        max_samples=config["experiment"].get("max_train_samples"),
    )
...
def run_epoch(model, loader, criterion, optimizer, device, threshold: float, train: bool) -> Dict[str, float]:
    model.train(train)
    avg = MetricAverager()
    loss_total = 0.0
    sample_count = 0
...
def main() -> None:
    ...
    for epoch in range(1, int(config["train"]["epochs"]) + 1):
        ...
        train_metrics = run_epoch(
            ...
            threshold=config["train"].get("threshold", 0.5), train=True,
        )
        val_metrics = run_epoch(
            model, val_loader, criterion, optimizer, device,
            threshold=config["train"].get("threshold", 0.5), train=False,
        )
        ...
        if val_metrics["dice"] > best_dice:
            ...
            save_json({"best_epoch": epoch, "best_val_dice": best_dice}, output_dir / "best_metrics.json")
