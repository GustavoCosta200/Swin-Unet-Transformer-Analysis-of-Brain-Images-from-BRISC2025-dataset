...
from .dataset import BriscSegmentationDataset, collect_samples
...
def main() -> None:
    ...
    samples = collect_samples(paths["test_images"], paths["test_masks"])
    max_test = config["experiment"].get("max_test_samples")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(loader)):
            ...
            logits = model(images)
            metrics = segmentation_metrics(logits, masks, config["train"].get("threshold", 0.5))
            avg.update(metrics, n=images.size(0))
            rows.append({"batch": batch_idx, **metrics})

            preds = (torch.sigmoid(logits) >= config["train"].get("threshold", 0.5)).float()
            for i in range(images.size(0)):
                if saved >= args.save_images:
                    break
                save_prediction_grid(images[i], masks[i], preds[i], pred_dir / f"sample_{saved:03d}.png")
                saved += 1
